...
namespace Mnemonics
{
    ...
    std::tuple<std::string, Crypto::SecretKey> MnemonicToPrivateKey(std::vector<std::string> words)
    {
        ...
        if (len != 25)
        {
            ...
            return std::tuple<std::string, Crypto::SecretKey>(str.str(), key);
        }
        ...
        {
            return std::tuple<std::string, Crypto::SecretKey>("Mnemonic seed has incorrect checksum!", key);
        }
        ...
        for (size_t i = 0; i < words.size() - 1; i += 3)
        {
            ...
            if (!(val % wlLen == w1))
            {
                return std::tuple<std::string, Crypto::SecretKey>("Mnemonic seed is invalid!", key);
            }
        ...
        return std::tuple<std::string, Crypto::SecretKey>(std::string(), key);
    }
