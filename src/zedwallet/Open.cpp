                                                 ...
                                                 &wallet)
{
    ...
    while (true)
    {
        ...
        auto res = Mnemonics::MnemonicToPrivateKey(mnemonicPhrase);
        
        auto error = std::get<0>(res);
        auto privateSpendKey = std::get<1>(res);
...
std::shared_ptr<WalletInfo> generateWallet(CryptoNote::WalletGreen &wallet)
{
    ...
    CryptoNote::KeyPair privateViewKey;

    Crypto::generate_keys(spendKey.publicKey, spendKey.secretKey);
    Crypto::generate_keys(privateViewKey.publicKey, privateViewKey.secretKey);
    ...
    wallet.initializeWithViewKey(
        walletFileName, walletPass, privateViewKey.secretKey);//, 0, true   );
