              ...
              bool sendAll, std::string nodeAddress, uint32_t nodeFee)
{
    ...
    const uint64_t balanceNoDust = balance; //walletInfo->wallet.getBalanceMinusDust
    //(
    ...
    uint64_t mixin = 1; //CryptoNote::getDefaultMixinByHeight(height);
   ...
   // );
    ...
    if (balance < amount + fee + nodeFee)
    {
        ...
        return NotEnoughBalance;
         ...
         if (balanceNoDust < amount + WalletConfig::minimumFee + nodeFee)
    {
        std::cout << std::endl
