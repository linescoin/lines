...
void run(CryptoNote::WalletGreen &wallet, CryptoNote::INode &node,
         Config &config)
{
    auto res = selectionScreen(config, wallet, node);
    bool quit = std::get<0>(res);
    std::shared_ptr<WalletInfo> walletInfo = std::get<1>(res);
    ...
    if (!quit)
    {
        ...
        Tools::SignalHandler::install([&walletInfo, &node,
                                       &alreadyShuttingDown]
        {
